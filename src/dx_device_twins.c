...
void dx_deviceTwinUnsubscribe(void) {
	for (int i = 0; i < _deviceTwinCount; i++) { deviceTwinClose(_deviceTwins[i]); }
}
...
void dx__deviceTwinCallbackHandler(DEVICE_TWIN_UPDATE_STATE updateState, const unsigned char* payload,
...
static void setDesiredState(JSON_Object* jsonObject, DX_DEVICE_TWIN_BINDING* deviceTwinBinding) {
	...
	case DX_TYPE_DOUBLE:
		if (json_object_has_value_of_type(jsonObject, deviceTwinBinding->twinProperty, JSONNumber)) {
			*(double*)deviceTwinBinding->twinState = (double)json_object_get_number(jsonObject, deviceTwinBinding->twinProperty);
...
static bool deviceTwinReportState(DX_DEVICE_TWIN_BINDING* deviceTwinBinding, void* state, bool deviceTwinAcknowledgment, DX_DEVICE_TWIN_RESPONSE_CODE statusCode) {
	...
	case DX_TYPE_DOUBLE:
		*(double*)deviceTwinBinding->twinState = *(double*)state;
		...
		if (deviceTwinAcknowledgment) {
			len = snprintf(reportedPropertiesString, reportLen, "{\"%s\":{\"value\":%lf, \"ac\":%d, \"av\":%d}}",
