...
static bool initialise_inter_core_communications(INTERCORE_CONTEXT *intercore_ctx)
{
    ...
    int result = setsockopt(intercore_ctx->sockFd, SOL_SOCKET, SO_RCVTIMEO, &recvTimeout,
                            sizeof(recvTimeout));
    if (result == -1) {
...
/// </summary>
bool dx_interCoreSendMessage(INTERCORE_CONTEXT *intercore_ctx, void *control_block,
                             size_t message_length)
{
    ...
    int bytesSent = send(intercore_ctx->sockFd, control_block, message_length,
                         intercore_ctx->nonblocking_io ? MSG_DONTWAIT : 0);
    if (bytesSent == -1) {
...
static bool ProcessMsg(INTERCORE_CONTEXT *intercore_ctx)
{
    ...
    ssize_t bytesReceived = recv(intercore_ctx->sockFd, (void *)intercore_ctx->intercore_recv_block,
                                 intercore_ctx->intercore_recv_block_length, 0);
